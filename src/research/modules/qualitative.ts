...
interface QualitativeOutput {
  team?: string;
  problem?: string;
  token_unlock_schedule?: string;
  airdrop?: string;
  narrative_alignment?: string;
  whitepaper_assessment?: string;
  red_flags?: string[];
}

type StringField = Exclude<keyof QualitativeOutput, "red_flags">;

const CRITERIA_META: Array<{ key: StringField; criterion: string; label: string }> = [
...
function naResults(reason: string): CriterionResult[] {
...
export async function analyzeQualitative(profile: ProjectProfile): Promise<CriterionResult[]> {
  ...
  const { primary, results } = await generateWithConsensus<QualitativeOutput>(
    QUALITATIVE_SYSTEM,
    `Project: ${profile.name} (${profile.symbol ?? "no ticker"})\n\n${sourceTexts.join("\n\n")}`
  );
