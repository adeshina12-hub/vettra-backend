...
async function getProtocols(): Promise<DefiLlamaProtocol[]> {
  if (protocolsCache && Date.now() - cacheAt < CACHE_MS) return protocolsCache;
  const res = await fetch("https://api.llama.fi/protocols");
  if (!res.ok) throw new Error(`DeFiLlama protocols fetch failed: ${res.status}`);
  protocolsCache = (await res.json()) as DefiLlamaProtocol[];
  cacheAt = Date.now();
...
async function tryNansenSmartMoney(profile: ProjectProfile): Promise<CriterionResult | null> {
  if (!config.nansen.apiKey) return null;
  ...
  try {
    ...
    const payload = (await res.json()) as { data?: NansenSmartMoneyRow[] };
    ...
    if (typeof netFlow7d !== "number") return null;
...
export async function analyzeOnchainGrowth(profile: ProjectProfile): Promise<CriterionResult> {
  try {
    ...
    const match = protocols.find(
      (p) =>
        (profile.coingeckoId && p.gecko_id === profile.coingeckoId) ||
    ...
    if (!match) {
      return {
        ...
        label: "On-chain Growth",
        findings:
          "No matching DeFiLlama protocol or Nansen smart-money token signal was found. This is expected for non-DeFi projects or tokens without a tracked TVL app — not necessarily a red flag.",
        confidence: "low",
      };
    }
