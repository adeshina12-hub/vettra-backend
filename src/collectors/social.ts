...
async function sorsaPost(path: string, body: Record<string, unknown>): Promise<any> {
  const res = await fetch(`${SORSA_BASE}${path}`, {
    method: "POST",
    headers: { ApiKey: config.social.sorsaApiKey, "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
  if (!res.ok) throw new Error(`Sorsa POST ${path} failed: ${res.status} ${await res.text()}`);
  return res.json();
}

async function sorsaGet(path: string, params: Record<string, string>): Promise<any> {
  const url = new URL(`${SORSA_BASE}${path}`);
  for (const [k, v] of Object.entries(params)) url.searchParams.set(k, v);
  const res = await fetch(url, { headers: { ApiKey: config.social.sorsaApiKey } });
  if (!res.ok) throw new Error(`Sorsa GET ${path} failed: ${res.status} ${await res.text()}`);
  return res.json();
}

function engagementScore(t: SorsaTweet): number {
  return t.likes_count + t.retweet_count * 2 + t.quote_count * 3 + t.reply_count;
}

function isRecent(t: SorsaTweet, hours: number): boolean {
...
async function getScoreCached(username: string): Promise<number | null> {
  if (scoreCache.has(username)) return scoreCache.get(username)!;
  try {
...
export async function collectSocialSignals(watchTerms: string[]): Promise<RawSignal[]> {
  ...
  for (const term of watchTerms) {
    try {
      ...
      const tweets: SorsaTweet[] = data.tweets ?? [];
      ...
      for (const t of topTweets) {
        if (engagementScore(t) < 50) continue; // filter out low-engagement noise
