...
export async function analyzeCommunity(profile) {
    ...
    if (profile.coingeckoId) {
        try {
            const responseData = await fetchCoinGecko(`/coins/${profile.coingeckoId}?localization=false&tickers=false&market_data=false&community_data=true&developer_data=false`);
