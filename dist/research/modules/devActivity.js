...
export async function analyzeDevActivity(profile) {
    ...
    const headers = { "User-Agent": "web3-research-agent" };
    ...
    try {
        ...
        const activityRes = await fetch(`https://api.github.com/repos/${profile.githubRepo}/stats/commit_activity`, { headers });
        let last4WeeksCommits = null;
        if (activityRes.ok) {
            const activity = await activityRes.json();
            if (Array.isArray(activity)) {
                last4WeeksCommits = activity.slice(-4).reduce((sum, w) => sum + (w.total ?? 0), 0);
            }
        }
