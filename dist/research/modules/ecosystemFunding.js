...
function extractFundingText(raw) {
    ...
    const matches = Array.from(new Set(raw
        .split(/(?<=[.!?])\s+/)
        .filter((sentence) => detectors.some((re) => re.test(sentence)))
        .slice(0, 5)
        .map((sentence) => sentence.trim())));
    if (matches.length === 0)
        return "No explicit funding or raise language found in the public sources.";
    return matches.join(" ");
}
function extractBackerText(raw) {
    const patterns = [
    ...
    if (sentences.length === 0)
        return "No explicit backer or investor language found in the public sources.";
    return sentences.join(" ");
}
async function tryDuneFunding(profile) {
...
export async function analyzeEcosystemFunding(profile) {
    const sourceTexts = [];
