...
export async function synthesizeReport(profile, criteria) {
    const findingsBlock = criteria
        .map((c) => `## ${c.label} (confidence: ${c.confidence})\n${c.findings}`)
        .join("\n\n");
    const { primary, results } = await generateWithConsensus(SYNTHESIS_SYSTEM, `Project: ${profile.name} (${profile.symbol ?? "no ticker"})\n\n${findingsBlock}`);
    if (!primary) {
        return {
            overallScore: 0,
