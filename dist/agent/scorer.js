...
export async function scoreSignals(signals) {
    ...
    if (!config.anthropicApiKey) {
        console.warn(`[agent] ANTHROPIC_API_KEY not set — skipping scoring for ${signals.length} signal(s). ` +
    ...
    try {
        ...
        console.error(`[agent] Anthropic call failed — skipping scoring for this run (collectors/storage are unaffected). ` +
            `Check ANTHROPIC_API_KEY in .env if this persists:`, err);
        return [];
    }
